...
struct MockFileLoader {
    static func load(_ filename: String, _ fileExt: String = "json") -> [DomainModel.ToDo] {
        ...
        guard let file = Bundle.module.url(forResource: filename, withExtension: fileExt) else {
            let message = "\(Current.logHeader()) Couldn't find \(filename) in module bundle (in Resources folder)"
        ...
        do {
            ...
            let message = "\(Current.logHeader()) Couldn't load \(fullName) from module bundle:\n\(error)"
            Logger.storage.critical("\(message)")
            fatalError(message)
        }
        ...
        do {
            ...
            Logger.storage.info("\(Current.logHeader()) Parsed \(fullName) and loaded \(DTOModel.ToDos.self)")
            return try result.todos.map { try ToDoDomainMapper.toDomain($0) }
        } catch {
            let message = "\(Current.logHeader()) Couldn't parse \(fullName) as \(DTOModel.ToDos.self):\n\(error)"
            Logger.storage.critical("\(message)")
            fatalError(message)
        }
    }
